...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (action === 'stats') {
      ...
      const r = await fetch(
        URL + '/rest/v1/referrals?referrer_phone=eq.' + encodeURIComponent(phone) + '&select=converted,reward_type',
        { headers: headers() }
      );
      if (!r.ok) {
        const txt = await r.text();
      ...
      const conversions = rows.filter(x => x.converted === true).length;
      // 500 F pour un filleul Pro, 1000 F pour un Super PRO
      const gains = rows.reduce((s, x) => {
        if (!x.converted) return s;
        return s + (x.reward_type === 'superpro' ? 1000 : 500);
      }, 0);
